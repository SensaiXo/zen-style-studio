...
const Blog = () => {
  ...
  return (
    <section id="blog" className="py-20 bg-gradient-to-br from-muted to-zen-light/10">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogPosts.map((post, index) => (
            <article
              key={index}
              className="bg-white rounded-3xl overflow-hidden shadow-zen-soft hover:shadow-zen transition-all duration-300 hover:-translate-y-1 group"
              data-aos="fade-up"
              data-aos-delay={index * 100}
            >
              {/* Featured Image Placeholder */}
              <div className="h-48 bg-gradient-zen relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-zen-blue/20 to-zen-mint/20"></div>
              ...
              <div className="p-6">
                {/* Meta info */}
                <div className="flex items-center gap-4 text-sm text-zen-gray mb-4">
                ...
                {/* Title */}
                <h3 className="text-xl font-semibold text-zen-dark mb-3 group-hover:text-primary transition-colors duration-300">
                  {post.title}
                </h3>

                {/* Excerpt */}
                <p className="text-zen-gray mb-4 leading-relaxed">
                  {post.excerpt}
                </p>

                {/* Read more */}
                <div className="flex items-center justify-between">
                  <span className="text-sm text-zen-gray">{post.readTime}</span>
                  <a
                    href="#"
                    className="inline-flex items-center gap-2 text-primary font-medium hover:text-zen-blue transition-colors duration-300 group-hover:gap-3"
                  >
                    Read More
